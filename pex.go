...
// Format: "PEX:<region>:<nodeID>:<addr1>,<addr2>,...:<signedAt RFC3339>"
//
// Region is part of the signed prefix so a peer cannot spoof another
// peer's region by re-signing a captured PEX entry with the same
// (nodeID, addresses, signedAt) tuple and a different region.
func pexSignaturePayload(nodeID, region string, addresses []string, signedAt time.Time) []byte {
	msg := fmt.Sprintf("PEX:%s:%s:", region, nodeID)
...
// is part of the signed payload — a peer cannot forge another peer's
// region by re-publishing its entry with a tampered Region field.
...
// VerifyPEXEntry checks the ed25519 signature on a PEX entry. The
// publicKey must correspond to the entry's NodeID (looked up from
// LAD member records).
func VerifyPEXEntry(entry PEXEntry, publicKey ed25519.PublicKey) bool {
	if len(entry.Signature) == 0 || len(publicKey) == 0 {
		return false
	}
	payload := pexSignaturePayload(entry.NodeID, entry.Region, entry.Addresses, entry.SignedAt)
